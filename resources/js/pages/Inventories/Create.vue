        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-muted-foreground">Add a new item to inventory</p>
                </div>
            </div>

            <div class="max-w-2xl">
                <form @submit.prevent="form.post(inventoryStore().url)" class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-sm font-medium">Item Name</label>
                        <Input v-model="form.item_name" placeholder="Enter item name" />
                        <p v-if="form.errors.item_name" class="text-sm text-red-600">{{ form.errors.item_name }}</p>
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium">Description</label>
                        <Textarea v-model="form.description" placeholder="Enter item description" />
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="space-y-2">
                            <label class="text-sm font-medium">Type of Supply</label>
                            <Select v-model="form.type_of_supply">
                                <SelectTrigger>
                                    <SelectValue placeholder="Select type of supply" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem v-for="(label, value) in props.typesOfSupply" :key="value" :value="value">
                                        {{ label }}
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                            <p v-if="form.errors.type_of_supply" class="text-sm text-red-600">{{ form.errors.type_of_supply }}</p>
                        </div>

                        <div class="space-y-2">
                            <label class="text-sm font-medium">Supplier</label>
                            <Input v-model="form.supplier" placeholder="Enter supplier name" />
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="space-y-2">
                            <label class="text-sm font-medium">Cost per Unit ($)</label>
                            <Input v-model="form.cost_per_unit" type="number" step="0.01" min="0" />
                            <p v-if="form.errors.cost_per_unit" class="text-sm text-red-600">{{ form.errors.cost_per_unit }}</p>
                        </div>
