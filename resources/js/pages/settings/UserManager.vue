...
import { Plus, Search, Eye, Edit, Trash2, UserCheck, UserX } from 'lucide-vue-next';
...
const toggleUserStatus = (userId: number) => {
    router.post(`/settings/users/${userId}/toggle-status`, {}, {
        preserveScroll: true,
    });
};
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            ...
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                ...
                <div class="rounded-lg border bg-card p-4">
                    <div class="text-2xl font-bold">{{ props.stats.active_users }}</div>
                    <p class="text-sm text-muted-foreground">Active Users</p>
                </div>
                <div class="rounded-lg border bg-card p-4">
                    <div class="text-2xl font-bold">{{ props.stats.staff_count }}</div>
                    <p class="text-sm text-muted-foreground">Staff</p>
                </div>
                <div class="rounded-lg border bg-card p-4">
                    <div class="text-2xl font-bold">{{ props.stats.doctor_count }}</div>
                    <p class="text-sm text-muted-foreground">Doctors</p>
                </div>
                <div class="rounded-lg border bg-card p-4">
                    <div class="text-2xl font-bold">{{ props.stats.patient_count }}</div>
            ...
            <div class="flex flex-wrap items-center gap-4">
                ...
                <Select v-model="typeFilter" @update:model-value="updateFilters">
                ...
                <Select v-model="statusFilter" @update:model-value="updateFilters">
                    <SelectTrigger class="w-40">
                    ...
                    <TableBody>
                        <TableRow v-for="user in filteredUsers" :key="user.id">
                            ...
                            <TableCell>
                                ...
                                <span v-else-if="user.department_name">{{ user.department_name }}</span>
                            ...
                            <TableCell>
                                <div class="flex items-center gap-2">
                                    ...
                                    >
                                        <UserX v-if="user.status === 'active'" class="size-4" />
