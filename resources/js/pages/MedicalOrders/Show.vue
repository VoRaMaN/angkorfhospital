...
import { ArrowLeft, Edit, FlaskConical, Syringe, Scan, UserCheck, Activity, Package, ChevronDown, ChevronRight, Pill, Play, CheckCircle, FileText } from 'lucide-vue-next';
import { index, edit, process as processRoute, complete as completeRoute } from '@/routes/medical-orders';
    ...
    props.medicalOrder.order_items.forEach((item) => {
        ...
        if (item.item_type === 'lab' && item.inventory_item) {
            ...
            const panel = props.labPanels.find(p =>
                p.items.some(panelItem => panelItem.id === item.inventory_item!.id)
            );
            if (panel) {
...
const confirmProcess = () => {
    if (confirm('Are you sure you want to start processing this medical order?')) {
        // Use router.patch to call the process route
        router.patch(processRoute(props.medicalOrder.id).url, {}, {
            onSuccess: () => {
        ...
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-muted-foreground">View comprehensive medical order information</p>
                </div>
            ...
            <div class="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Lab tests, procedures, imaging, and supplies</CardDescription>
                    </CardHeader>
                    <CardContent>
                                ...
                                <div class="space-y-2 ml-8">
                                    <div v-for="itemData in items" :key="itemData.item.id"
                                        ...
                                        <div class="flex items-center justify-between p-4 cursor-pointer hover:bg-muted/50 transition-colors"
                                            @click="toggleItemExpansion(itemData.item.id)">
                                            <div class="flex items-center gap-3">
                                                <component :is="getItemTypeIcon(itemData.item.item_type)"
                                                    class="size-6 text-primary" />
                                                <div>
                                                    <div class="flex items-center gap-2">
                                                        <span class="font-medium">{{ itemData.item.item_name }}</span>
                                                        <Badge variant="outline" class="text-xs">
                                                            {{ getItemTypeLabel(itemData.item.item_type) }}
                                                        </Badge>
                                                        <Badge v-if="hasAdditionalInfo(itemData.item)"
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <Badge :class="getStatusColor(itemData.item.status)">
                                                    {{ itemData.item.status_label }}
                                                </Badge>
                                                <component
                                                    :is="expandedItems.has(itemData.item.id) ? ChevronDown : ChevronRight"
                                                    class="size-4 text-muted-foreground" />
                                            </div>
                                        </div>

                                        <!-- Collapsible Content -->
                                        <div v-if="expandedItems.has(itemData.item.id)"
                                            class="border-t bg-muted/20 px-4 py-3 space-y-3">
                                            <!-- Details -->
                                            <div v-if="itemData.item.details" class="text-sm">
                                                <span class="text-muted-foreground">Details:</span>
                                                <p class="mt-1">{{ itemData.item.details }}</p>
                                            </div>

                                            <!-- Medical Details -->
                                            <div v-if="itemData.item.dosage || itemData.item.frequency || itemData.item.route"
                                                class="grid gap-4 md:grid-cols-3 text-sm">
                                                <div v-if="itemData.item.dosage">
                                                    <span class="text-muted-foreground">Dosage:</span>
                                                    <span class="ml-1 font-medium">{{ itemData.item.dosage }}</span>
                                                </div>
                                                <div v-if="itemData.item.frequency">
                                                    <span class="text-muted-foreground">Frequency:</span>
                                                    <span class="ml-1 font-medium">{{ itemData.item.frequency }}</span>
                                                </div>
                                                <div v-if="itemData.item.route">
                                                    <span class="text-muted-foreground">Route:</span>
                                                    <span class="ml-1 font-medium">{{ itemData.item.route }}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-6 text-sm">
                                                <div>
                                                    ...
                                                    <span class="ml-1 font-medium">{{ itemData.item.quantity_required
                                                    }}</span>
                                                </div>
                                                <div v-if="itemData.item.completed_at">
                                                    <span class="text-muted-foreground">Completed:</span>
                                                    <span class="ml-1 font-medium">{{ itemData.item.completed_at
                                                    }}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div v-if="itemData.item.notes" class="text-sm">
                                                ...
                                                <p class="mt-1 italic">{{ itemData.item.notes }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <CardContent>
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <dd class="text-sm">{{ new Date(medicalOrder.created_at).toLocaleString() }}</dd>
                            </div>

                            <div class="space-y-2">
                                <dt class="text-sm font-medium text-muted-foreground">Last Updated</dt>
                                <dd class="text-sm">{{ new Date(medicalOrder.updated_at).toLocaleString() }}</dd>
                            </div>
                        </div>
                    </CardContent>
                </Card>
