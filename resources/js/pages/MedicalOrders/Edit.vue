...
import { ArrowLeft, Trash2, FlaskConical, Pill, Syringe, Scan, UserCheck, Activity, Package, Search, Plus, ChevronDown, ChevronRight } from 'lucide-vue-next';
...
const rxMedicinesByCategory = computed(() => {
    ...
    return props.rxMedicines.filter(med => med.category === activeRxCategory.value);
});
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-muted-foreground">Update medical order and its items</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Update basic details about this medical order</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-4">
                                    ...
                                    <FormControl>
                                        <Select v-model="form.patient_id">
                                            ...
                                            <SelectContent>
                                                ...
                                                <SelectItem v-for="patient in patients" :key="patient.id"
                                                    :value="patient.id">
                                                    {{ patient.name }}
                                                </SelectItem>
                                            </SelectContent>
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Select v-model="form.staff_id">
                                            ...
                                            <SelectContent>
                                                <SelectItem :value="null">None</SelectItem>
                                                <SelectItem v-for="staff in staff" :key="staff.id"
                                                    :value="staff.id">
                                                    {{ staff.name }}
                                                </SelectItem>
                                            </SelectContent>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <FormField name="priority">
                                <FormItem>
                                    ...
                                    <div v-if="form.errors.priority" class="text-sm text-destructive">
                        ...
                        <FormField name="order_details">
                            <FormItem>
                                ...
                                <FormControl>
                                    <Textarea v-model="form.order_details" placeholder="Enter order details" rows="3" />
                                </FormControl>
                                <div v-if="form.errors.order_details" class="text-sm text-destructive">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <FormField name="ordered_at">
                                <FormItem>
                                    ...
                                    <div v-if="form.errors.ordered_at" class="text-sm text-destructive">
                            ...
                            <FormField name="completed_at">
                                <FormItem>
                                    ...
                                    <div v-if="form.errors.completed_at" class="text-sm text-destructive">
                            ...
                            <FormField name="notes">
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Textarea v-model="form.notes" placeholder="Additional notes" rows="1" />
                                    </FormControl>
                ...
                <!-- Order Summary -->
                <Card v-if="orderSummary.total > 0" class="border-green-200 bg-green-50/50 dark:border-green-800 dark:bg-green-950/20">
                    <CardContent class="pt-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-right">
                                <div class="text-2xl font-bold text-green-800 dark:text-green-200">{{ orderSummary.total }}</div>
                                <div class="text-sm text-green-600 dark:text-green-400">{{ formatPrice(orderTotalPrice) }}</div>
                            </div>
                        </div>
                ...
                <Card>
                    <CardHeader>
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Update lab tests, procedures, imaging, and supplies</CardDescription>
                            </div>
                    ...
                    <CardContent class="space-y-4">
                            ...
                            <CardContent>
                                <div class="mb-4">
                                    <div class="relative">
                                        ...
                                        <Input v-model="labSearchQuery" placeholder="Search lab tests..." class="pl-10" />
                                    </div>
                                </div>
                                        ...
                                        <div class="space-y-2 max-h-96 overflow-y-auto">
                                            <div v-for="item in filteredLabItems" :key="item.id"
                                                ...
                                                <div class="text-right text-xs">
                                                    <div class="font-medium text-green-600 dark:text-green-400">{{ formatPrice(getLabItemPrice(item.id)) }}</div>
                                                    <span class="text-muted-foreground">Qty: {{
                                                        item.quantity_required }}</span>
                                                </div>
                                            </div>
                                            <div v-if="filteredLabItems.length === 0" class="text-center py-8 text-muted-foreground">
                        ...
                        <Card v-if="showRxDialog" class="border-primary">
                            ...
                            <CardContent>
                                <div class="mb-4">
                                    <div class="relative">
                                        <Search class="absolute left-3 top-1/2 transform -translate-y-1/2 size-4 text-muted-foreground" />
                                        <Input v-model="rxSearchQuery" placeholder="Search medicines..." class="pl-10" />
                                    </div>
                                </div>
                                    ...
                                    <TabsContent value="All" class="space-y-4">
                                        <div class="space-y-2 max-h-96 overflow-y-auto">
                                            <div v-for="medicine in filteredRxMedicines" :key="medicine.id"
                                                ...
                                                <div class="text-right text-xs">
                                                    <div class="text-muted-foreground">Stock: {{ medicine.quantity }}</div>
                                                    <div class="font-medium text-green-600 dark:text-green-400">{{ formatPrice(medicine.selling_price) }}</div>
                                                    <div v-if="medicine.category" class="text-muted-foreground">{{ medicine.category }}</div>
                                                </div>
                                            </div>
                                            <div v-if="filteredRxMedicines.length === 0" class="text-center py-8 text-muted-foreground">
                        ...
                        <div v-else class="space-y-6">
                            <div v-for="[type, items] in Object.entries(groupedOrderItems)" :key="type" class="space-y-3">
                                <!-- Group Header -->
                                <div class="flex items-center gap-3 pb-2 border-b">
                                    <component :is="getItemTypeIcon(items[0]?.panelName ? 'lab' : type)" class="size-5 text-primary" />
                                    <h3 class="font-semibold text-lg">{{ getItemTypeDisplayName(type, items[0]?.panelName) }}</h3>
                                    ...
                                    <div v-for="itemData in items" :key="itemData.index"
                                        ...
                                        <div class="flex items-center justify-between p-3 bg-muted/30 hover:bg-muted/50 cursor-pointer"
                                            ...
                                            <div class="flex items-center gap-3">
                                                <Button type="button" variant="ghost" size="sm" class="h-6 w-6 p-0" @click.stop="toggleItemExpansion(itemData.index)">
                                                    <component :is="expandedItems.has(itemData.index) ? ChevronDown : ChevronRight" class="size-4" />
                                                </Button>
                                                <div class="flex flex-col">
                                                    <span class="font-medium text-sm">{{ itemData.item.item_name || 'Unnamed item' }}</span>
                                                    <span v-if="itemData.item.details" class="text-xs text-muted-foreground truncate max-w-xs">{{ itemData.item.details }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-3">
                                                <div class="text-right">
                                                    <div class="text-sm font-medium">{{ formatPrice(getItemPrice(itemData.item)) }}</div>
                                                    <div v-if="itemData.item.quantity_required > 1" class="text-xs text-muted-foreground">
                                                        {{ itemData.item.quantity_required }} × {{ formatPrice(itemData.item.selling_price || 0) }}
                                                    </div>
                                                </div>
                                                <div class="flex gap-1">
                                                    <Button type="button" variant="ghost" size="sm" @click.stop="duplicateOrderItem(itemData.index)" title="Duplicate item">
                                                        <Plus class="size-4 text-muted-foreground" />
                                                    </Button>
                                                    <Button type="button" variant="ghost" size="sm" @click.stop="removeOrderItem(itemData.index)">
                                        ...
                                        <div v-if="expandedItems.has(itemData.index)" class="p-3 space-y-3 border-t">
                                            ...
                                            <div v-if="itemData.item.item_type === 'lab'" class="flex gap-3 items-end">
                                                <div class="flex-1">
                                                    ...
                                                    <Input v-model="itemData.item.item_name" placeholder="Test name" readonly class="bg-muted h-8" />
                                                </div>
                                                <div class="flex-1">
                                                    <Label class="text-xs">Details</Label>
                                                    <Input v-model="itemData.item.details" placeholder="Test parameters, instructions" class="h-8" />
                                                </div>
                                                <div class="w-32">
                                                    <Label class="text-xs">Notes</Label>
                                                    <Input v-model="itemData.item.notes" placeholder="Additional notes" class="h-8" />
                                                </div>
                                            </div>
                                            ...
                                            <div v-if="itemData.item.item_type === 'rx_medicine'" class="space-y-2">
                                                <div class="flex gap-3 items-end">
                                                    <div class="flex-1">
                                                        ...
                                                        <Input v-model="itemData.item.item_name" placeholder="Medicine name" readonly class="bg-muted h-8" />
                                                    </div>
                                                    <div class="w-24">
                                                        <Label class="text-xs">Dosage</Label>
                                                        <Input v-model="itemData.item.dosage" placeholder="500mg" class="h-8" />
                                                    </div>
                                                    <div class="w-32">
                                                        <Label class="text-xs">Frequency</Label>
                                                        <Input v-model="itemData.item.frequency" placeholder="Twice daily" class="h-8" />
                                                    </div>
                                                    <div class="w-32">
                                                    ...
                                                    <div class="w-20">
                                                        ...
                                                        <Input v-model="itemData.item.quantity_required" type="number" min="1" placeholder="1" class="h-8" />
                                                    </div>
                                                </div>
                                                <div class="flex gap-3 items-end">
                                                    <div class="flex-1">
                                                        <Label class="text-xs">Instructions</Label>
                                                        <Input v-model="itemData.item.details" placeholder="Special instructions" class="h-8" />
                                                    </div>
                                                    <div class="w-32">
                                                        <Label class="text-xs">Notes</Label>
                                                        <Input v-model="itemData.item.notes" placeholder="Additional notes" class="h-8" />
                                            ...
                                            <!-- Procedure Fields -->
                                            <div v-if="itemData.item.item_type === 'procedure'" class="flex gap-3 items-end">
                                                <div class="flex-1">
                                                    <Label class="text-xs">Procedure Name *</Label>
                                                    <Input v-model="itemData.item.item_name" placeholder="Enter procedure name" class="h-8" />
                                                </div>
                                                <div class="flex-1">
                                                    <Label class="text-xs">Details</Label>
                                                    <Input v-model="itemData.item.details" placeholder="Procedure details, requirements" class="h-8" />
                                                </div>
                                                <div class="w-32">
                                                    <Label class="text-xs">Notes</Label>
                                                    <Input v-model="itemData.item.notes" placeholder="Additional notes" class="h-8" />
                                            ...
                                            <!-- Imaging Fields -->
                                            <div v-if="itemData.item.item_type === 'imaging'" class="flex gap-3 items-end">
                                                <div class="flex-1">
                                                    <Label class="text-xs">Imaging Type *</Label>
                                                    <Input v-model="itemData.item.item_name" placeholder="e.g., Chest X-Ray, CT Scan, MRI" class="h-8" />
                                                </div>
                                                <div class="flex-1">
                                                    <Label class="text-xs">Details</Label>
                                                    <Input v-model="itemData.item.details" placeholder="Body part, with/without contrast, views" class="h-8" />
                                                </div>
                                                <div class="w-32">
                                                    <Label class="text-xs">Notes</Label>
                                                    <Input v-model="itemData.item.notes" placeholder="Additional notes" class="h-8" />
                                            ...
                                            <!-- Supply Fields -->
                                            <div v-if="itemData.item.item_type === 'supply'" class="flex gap-3 items-end">
                                                <div class="flex-1">
                                                    <Label class="text-xs">Supply Item</Label>
                                                    <Select @update:modelValue="(val) => selectInventoryItem(itemData.item, Number(val))">
                                                        <SelectTrigger class="h-8">
                ...
                <div class="flex gap-4">
                    <Button type="submit" :disabled="form.processing">
                        {{ form.processing ? 'Updating...' : 'Update Medical Order' }}
                    </Button>
                    <Button variant="outline" as-child>
