...
import { index as labPanelIndex, update as labPanelUpdate } from '@/routes/lab-panels';
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-muted-foreground">Update laboratory test panel information</p>
                </div>
            </div>

            <div class="max-w-4xl">
                <form @submit.prevent="form.put(labPanelUpdate(props.labPanel.id).url)" class="space-y-6">
                    <!-- Basic Information -->
                    <Card>
                        <CardHeader>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div v-if="form.inventory_items.length === 0" class="text-center py-8 text-muted-foreground">
                                No inventory items added yet. Click "Add Item" to get started.
                            </div>

                            <div v-else class="space-y-4">
                                <div v-for="(item, index) in form.inventory_items" :key="index" class="border rounded-lg p-4">
                                    <div class="flex items-start justify-between mb-4">
                                        <h4 class="font-medium">Item {{ index + 1 }}</h4>
                                        <Button type="button" variant="outline" size="sm" @click="removeInventoryItem(index)">
                                            <Trash2 class="size-4" />
                                        </Button>
                                    </div>
                                    ...
                                    <div class="grid gap-4 md:grid-cols-3">
                                        <div class="space-y-2">
                                            ...
                                            <Select v-model="item.inventory_id">
                                                ...
                                                <SelectContent>
                                                    <SelectItem v-for="supply in getAvailableSupplies()" :key="supply.id" :value="supply.id">
                                                        {{ supply.item_name }} ({{ supply.quantity }} {{ supply.unit }} available)
                                                    </SelectItem>
                                                    <SelectItem v-for="supply in props.labSupplies.filter(s => s.id === item.inventory_id)" :key="supply.id" :value="supply.id">
                                                        {{ supply.item_name }} ({{ supply.quantity }} {{ supply.unit }} available) - Currently Selected
                                                    </SelectItem>
                                                </SelectContent>
                                            </Select>
                                        </div>
                            ...
                            <p v-if="form.errors['inventory_items']" class="text-sm text-red-600 mt-2">{{ form.errors['inventory_items'] }}</p>
                        </CardContent>
                    </Card>
